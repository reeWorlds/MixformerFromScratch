...
vector <array<float, 3> > res_output(NUM_THREADS);
...
public:
	...
	Config(int id)
	{
		...
		scale = 50.0;
		octaves = 6;
		persistance = 0.4 + g_unif_rng(g_rng) * 0.17;
		lacunarity = 1.85 + g_unif_rng(g_rng) * 0.3;

		extraSeed = id + 1e9;
		extraScale = 25.0;
		extraOctaves = 1;
		extraPersistance = 0.4 + g_unif_rng(g_rng) * 0.17;
		extraLacunarity = 1.85 + g_unif_rng(g_rng) * 0.3;

		searchSize = search_resolution;

		targetExtraNoiseCoef = 0.02 + g_unif_rng(g_rng) * 0.03;
		targetSize = target_resolution;
		targetLen = 75 + g_rng() % 100;
	}
};
...
void gen_res_images(Config config, int thread_id)
{
	...
	scale = (rectFullSize - 75) / 100.0;

	res_output[thread_id][0] = cx;
	res_output[thread_id][1] = cy;
	res_output[thread_id][2] = scale;
}
...
int main()
{
	...
	for (int packet_i = 0; packet_i < num_paches; packet_i++)
	{
		...
		float* all_output = new float[3 * patch_size];
...
#pragma omp parallel for num_threads(NUM_THREADS)
		...
		file_output.write((char*)all_output, 3 * patch_size * sizeof(float));
		file_output.close();
